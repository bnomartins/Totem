<template>
  ...
  <Modal :show="confirmar" @close="closeModal">
    ...
    <div class="py-8 border-b border-solid border-slate-200 rounded-t text-gray-600">
      ...
      <form @submit.prevent="editarClinico()" class="w-full">
          <div class="m-2 p-2 rounded">
              <label class="font-bold text-xl text-gray-600" for="diagnostico">Diagnostico</label>
              <input type="text" placholder="Digite o diagnostico" v-model="form.diagnostico"
  ...
  const props = defineProps({
    ...
    const form = useForm({
      id: props.clinico.id,
      diagnostico: props.clinico.diagnostico,
      observacao:  props.clinico.observacao,
      tipo: 'Edição',
