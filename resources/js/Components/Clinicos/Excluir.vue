<template>
    ...
    <Modal :show="confirmar" @close="closeModal">
      ...
      <div class="py-8 border-b border-solid border-slate-200 rounded-t text-gray-600">
        <h1 class="text-center m-auto text-3xl font-bold" >
          Os dados do clinico serão removidos permanentemente, você deseja continuar?
        </h1>
      </div>
        ...
        <div class="mx-2 px-6 border rounded text-xl w-full">
            ...
            <form @submit.prevent="excluirClinico(form.id)" class="w-full">
    ...
    function excluirClinico(id){
        form.delete(route('clinicos.destroy', id),{
            onSuccess: () => (
              closeModal()
              ),
        })
