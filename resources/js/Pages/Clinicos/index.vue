    ...
    <AuthenticatedLayout>
        ...
        <div class="m-auto bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="m-5 border-b border-gray-200">
                    ...
                    <div class="mx-auto w-full whitespace-no-wrap">
                        ...
                        <div class="bg-gray-100 shadow mx-auto">
                            ...
                            <tr class="bg-gray-900 text-white">
                                ...
                                <th class="px-2 py-2 text-2xl font-bold text-left">Diagnostico</th>
                                <th class="px-2 py-2 text-2xl font-bold text-left">Observacção</th>
                                <th class="px-2 py-2 text-2xl font-bold text-left">Ações</th>
                            </tr>
                            <tr v-for="(p, index) in clinicos.data" :key="p.id"
                                :class="{'bg-gray-300': index%2===0}">
                                <td class="px-2 py-2 text-2xl text-left">{{p.id}}</td>
                                <td class="px-2 py-2 text-2xl text-left">{{p.diagnostico}}</td>
                                <td class="px-2 py-2 text-2xl text-left">{{p.observacao}}</td>
                                <td class="px-2 py-2 text-xl float-right">
                                    <EdicaoTriagem
                                        :clinico="{id: p.id, diagnostico: p.diagnostico, observacao: p.observacao}"
                                    />
                                    ...
                                    @click.prevent="excluirTriagem(p.id)"
                                    >
                                        Excluir
                                    </button> -->
                                </td>
                ...
                <div class="m-2 p-2">
                    <Pagination :links="clinicos.links"/>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
